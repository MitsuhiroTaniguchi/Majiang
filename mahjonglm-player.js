...
async function generate(tokens, allowed, n, serverUrl) {
    ...
    if (process.env.DEBUG_GEN) {
        for (const g of res.generated) {
            if (g.token.startsWith('discard_'))
                console.log(`  [GEN] ${g.token} (p=${g.prob}) allowed=${(allowed||[]).length}`);
        }
    }
...
const DORA_HUPAI = new Set(["ドラ", "裏ドラ", "赤ドラ"]);
...
class MahjongLMPlayer extends Majiang.Player {
    ...
    action_kaiju(kaiju) {
        ...
        const ruleLen = "rule_length_hanchan";
        this._seq = [
            "<bos>",
            `rule_player_${this._seatCount}`,
            ruleLen,
    ...
    action_qipai(qipai) {
        ...
        for (let s = 0; s < this._seatCount; s++) {
    ...
    _resolveAnkan(seat, kanTile) {
        ...
        console.log(`  [MahjongLM] 暗槓:${gang[0]}`);
    ...
    async _resolveDiscard(seat, dapai) {
        if (process.env.DEBUG_HAND) {
            const xi = Majiang.Util.xiangting(this.shoupai);
            console.log(`  [HAND] ${this.shoupai.toString()} xi=${xi}`);
        }
        const allowed = [];
    ...
    action_dapai(dapai) {
        ...
        if (this.allow_hule(this.shoupai, rongpai)) {
            ...
            if (this.get_chi_mianzi(this.shoupai, p).length > 0) allReactOpts.push("chi");
    ...
    async _asyncReaction(dapai) {
        try {
            ...
            if (chiMelds.length === 0 && pengMelds.length === 0 && gangMelds.length === 0) {
                if (this.allow_no_daopai(this.shoupai)) {
                    return this._callback({ daopai: "-" });
                }
                return this._callback();
            ...
            if (chiMelds.length > 0) reactOpts.push("chi");
            ...
            if (taken === "pon") {
                ...
                meld = await this._resolveChi(mySeat, chiMelds, dapai);
            }
    ...
    async _resolveChi(seat, melds, dapai) {
        if (melds.length === 1) {
            const pos = this._chiPos(melds[0], dapai);
            if (pos) this._seq.push(`chi_pos_${pos}`);
            const hasRed = this._meldHasRedFive(melds[0]);
            if (hasRed !== null) this._seq.push(hasRed ? "red_used" : "red_not_used");
            return melds[0];
        }
        ...
        const posMap = {};
        for (const m of melds) {
            const pos = this._chiPos(m, dapai);
            if (pos && !posMap[pos]) {
                posAllowed.push(`chi_pos_${pos}`);
                posMap[pos] = [];
            }
            if (pos) posMap[pos].push(m);
        }
        ...
        if (candidates.length === 1) {
            const hasRed = this._meldHasRedFive(candidates[0]);
            if (hasRed !== null) this._seq.push(hasRed ? "red_used" : "red_not_used");
            return candidates[0];
        }
    ...
    _chiPos(meld, dapai) {
        const calledTile = normTile(dapai.p);
        const calledIdx = tileIndex(calledTile);
        const tiles = [];
        const stripped = meld.replace(/[+\-=]/g, "");
        let suit = "";
        for (const ch of stripped) {
            if ("mpsz".includes(ch)) suit = ch;
            else tiles.push(suit + ch);
        }
        const indices = tiles.map(tileIndex);
        const sorted = [...indices].sort((a, b) => a - b);
        if (calledIdx === sorted[0]) return "high";
        if (calledIdx === sorted[1]) return "mid";
        if (calledIdx === sorted[2]) return "low";
        return null;
    }
    ...
    _appendMeldDetails(meld, action) {
        if (action === "chi") {
            const pos = this._chiPosFromMeld(meld);
            if (pos) this._seq.push(`chi_pos_${pos}`);
        }
        if (action === "chi" || action === "pon") {
            const red = this._meldHasRedFive(meld);
            if (red !== null) this._seq.push(red ? "red_used" : "red_not_used");
        }
    }

    _chiPosFromMeld(meld) {
        let suit = "";
        let calledIdx = -1;
        const indices = [];
        for (let i = 0; i < meld.length; i++) {
            const ch = meld[i];
            if ("mpsz".includes(ch)) { suit = ch; continue; }
            if (ch >= "0" && ch <= "9") {
                const idx = tileIndex(suit + ch);
                indices.push(idx);
                if (i + 1 < meld.length && "+-=".includes(meld[i + 1])) {
                    calledIdx = idx;
                }
            }
        }
        if (calledIdx < 0 || indices.length !== 3) return null;
        const sorted = [...indices].sort((a, b) => a - b);
        if (calledIdx === sorted[0]) return "high";
        if (calledIdx === sorted[1]) return "mid";
        if (calledIdx === sorted[2]) return "low";
    ...
    _classifyFulou(meld) {
        ...
        if (tiles.length === 4) return "minkan";
        const sorted = [...new Set(tiles)].sort((a, b) => a - b);
        if (sorted.length === 1) return "pon";
        return "chi";
    }
    ...
    action_gang(gang) {
        ...
        this._lastKanType = isAnkan ? 'ankan' : 'kakan';
    ...
    action_hule(hule) {
        ...
        if (process.env.DEBUG_SEQ) {
            console.log(`\n=== TOKEN SEQ (hule end, ${this._seq.length} tokens) ===`);
            console.log(this._seq.join(' '));
            console.log('=== END SEQ ===\n');
        }
        this._callback();
    }
    ...
    action_pingju(pingju) {
        ...
        this._seq.push("round_end");
        if (process.env.DEBUG_SEQ) {
            console.log(`\n=== TOKEN SEQ (round end, ${this._seq.length} tokens) ===`);
            console.log(this._seq.join(' '));
            console.log('=== END SEQ ===\n');
        }
        this._callback();
...
module.exports = MahjongLMPlayer;
